...
export default function SubmitExpenseWebScreen() {
  ...
  const markers = useRef<(google.maps.marker.AdvancedMarkerElement | null)[]>([null, null]);
  ...
  const [points, setPoints] = useState<(google.maps.LatLngLiteral | null)[]>([null, null]);
  ...
  useEffect(() => {
    ...
    const initMap = () => {
      ...
      const setupAutocomplete = (input: HTMLInputElement | null, index: number) => {
        if (!input) return;
        ...
        autocomplete.addListener("place_changed", () => {
          ...
          if (place.geometry?.location) {
            ...
            index === 0 ? setFromAddress(place.formatted_address || '') : setToAddress(place.formatted_address || '');
  ...
  const reverseGeocode = (latLng: google.maps.LatLngLiteral, index: number) => {
    ...
    geocoder.current.geocode({ location: latLng }, (results, status) => {
      if (status === google.maps.GeocoderStatus.OK && results?.[0]) {
        ...
        index === 0 ? setFromAddress(address || '') : setToAddress(address || '');
      }
    });
  };
  ...
  useEffect(() => {
    if (points[0] && points[1]) {
      calculateDistance(points[0] as google.maps.LatLngLiteral, points[1] as google.maps.LatLngLiteral);
  ...
  const handleSubmit = async () => {
    ...
    if (dist === 0.00 || !formPurpose.trim() || !formDate || !fromAddress || !toAddress || !formCompany || !formName || !formContactNumber.trim() || !formFromTime || !formToTime) {
    ...
    try {
      ...
      if (businessCardFile) {
        ...
        const storageRef = ref(storage, `business-cards/${Date.now()}_${businessCardFile.name}`);
          ...
          <ScrollView contentContainerStyle={styles.scrollContent}>
            <View style={styles.formContainer}>
              ...
              <View style={styles.inputRow}>
                ...
                <Text style={styles.fieldValue} numberOfLines={1}>{fromAddress || 'Not set'}</Text>
              </View>
              <View style={[styles.inputRow, { marginTop: 10 }]}>
                <Text style={styles.fieldLabel}>To:</Text>
                <Text style={styles.fieldValue} numberOfLines={1}>{toAddress || 'Not set'}</Text>
              </View>
              <View style={[styles.inputRow, { marginTop: 10 }]}>
                <Text style={styles.fieldLabel}>Distance:</Text>
                <Text style={styles.fieldValue}>{getDistance()} km</Text>
              </View>
              <View style={[styles.inputRow, { marginTop: 10, alignItems: 'flex-start' }]}> {/* Changed to Purpose */}
              ...
              <View style={[styles.inputRow, { marginTop: 10, alignItems: 'flex-start' }]}>
                <Text style={styles.fieldLabel}>Comapany/Site:</Text>
                <input
                  type="text"
                  value={formCompany}
                  onChange={(e) => setFormCompany(e.target.value)}
                  style={{ ...webStyles.input, flex: 1, maxWidth: 400, marginBottom: 0 }}
              ...
              <View style={[styles.inputRow, { marginTop: 10, alignItems: 'flex-start' }]}>
                ...
                <input
                  ...
                  onChange={(e) => setFormContactNumber(e.target.value.replace(/[^0-9]/g, ''))}
                  style={{ ...webStyles.input, flex: 1, maxWidth: 400, marginBottom: 0 }}
              ...
              <View style={[styles.inputRow, { marginTop: 10, alignItems: 'flex-start'  }]}>
                ...
                <input
                  ...
                  onChange={(e) => setFormParking(Math.max(0, parseFloat(e.target.value || '0')).toFixed(2))}
                  style={{ ...webStyles.input, flex: 1, maxWidth: 400, marginBottom: 0 }}
                  step="0.01"
                  placeholder="0.00"
                />
              ...
              <View style={[styles.inputRow, { marginTop: 10, alignItems: 'flex-start'  }]}>
                ...
                <input
                  ...
                  onChange={(e) => setFormToll(Math.max(0, parseFloat(e.target.value || '0')).toFixed(2))}
                  style={{ ...webStyles.input, flex: 1, maxWidth: 400, marginBottom: 0 }}
                  step="0.01"
                  placeholder="0.00"
                />
              ...
              <View style={[styles.inputRow, { marginTop: 10, alignItems: 'center' }]}>
              ...
              <TouchableOpacity 
                ...
                disabled={
                  getDistance() === 0 || !formPurpose.trim() || 
                  isNaN(parseFloat(formParking)) || isNaN(parseFloat(formToll))
                }
                style={[
                  styles.button, 
                  { marginTop: 10, alignSelf: 'flex-start' },
                  (getDistance() === 0 || !formPurpose.trim() || 
                   isNaN(parseFloat(formParking)) || isNaN(parseFloat(formToll))) && { opacity: 0.5 }
                ]}
              >
...
const styles = StyleSheet.create({
  ...
  header: { position: 'absolute', top: 10, left: 10, right: 10, zIndex: 10, padding: 15, backgroundColor: 'white', borderRadius: 8 },
